...
export default function BursarProtectedRoute({ children }) {
  const [status, setStatus] = useState('checking')

  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      const session = sessionStorage.getItem('bursarSession')

      if (!user || !session) {
        if (session) sessionStorage.removeItem('bursarSession')
        setStatus('denied')
        return
      }

      const result = await verifyRoleAccess(user, 'bursar')

      if (!result.allowed) {
        await logSecurityEvent({
          uid:         user.uid,
          action:      result.reason === 'WRONG_ROLE' ? 'WRONG_ROLE_ACCESS' : 'UNAUTHORISED_ROUTE_ACCESS',
          attemptedRole: 'bursar',
