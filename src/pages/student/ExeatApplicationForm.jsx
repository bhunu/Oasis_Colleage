...
const INPUT  = 'w-full bg-white/5 border border-white/10 focus:border-gold/50 focus:outline-none rounded-xl px-4 py-2.5 text-white font-montserrat text-sm placeholder-gray-600 transition-all'

export default function ExeatApplicationForm() {
  const navigate = useNavigate()
  const { studentData } = useStudent()
  const { termStartDate, termEndDate, hasDates } = useTermDates()
  ...
  const validate = () => {
    ...
    // Term boundary checks — only enforced when term dates are configured
    if (termStartDate || termEndDate) {
      const dep   = new Date(form.departureDate)
      const ret   = new Date(form.returnDate)
      const start = termStartDate ? new Date(termStartDate) : null
      const end   = termEndDate   ? new Date(termEndDate)   : null
      if (start && dep < start) {
        toast.error(`Departure date cannot be before term start (${fmtTermDate(termStartDate)})`)
        return false
      }
      if (end && dep > end) {
        toast.error(`Departure date cannot be after term end (${fmtTermDate(termEndDate)})`)
        return false
      }
      if (end && ret > end) {
        toast.error(`Return date cannot be after term end (${fmtTermDate(termEndDate)})`)
        return false
      }
    }
    if (docRequired && !file) {
      ...
      <div className="max-w-md mx-auto text-center space-y-5 py-16">
        ...
        <div>
          ...
          <p className="font-montserrat text-sm text-gray-400 leading-relaxed mt-2">
            You already have a <span className="text-amber-300 font-semibold">{activeExeat.status ?? 'pending'}</span> exeat application
            ({activeExeat.reason}). You cannot submit a new one until it is resolved.
          </p>
        </div>
        <button
          onClick={() => navigate('/student/exeat/my-applications')}
          className="font-montserrat text-xs bg-gold text-navy font-bold px-5 py-2.5 rounded-xl"
        >
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {/* Warn when term dates not configured — date validation will be skipped */}
      {!hasDates && (
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 text-xs text-amber-300 font-montserrat">
          Term dates are not configured. Contact your admin to set them in Portal Settings.
        </div>
      )}
      ...
      {(termStartDate || termEndDate) && (
        <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3">
          <MdCalendarMonth className="text-gold text-lg shrink-0" />
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-navy-800 border border-white/10 rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            <label className="flex flex-col items-center gap-2 border-2 border-dashed border-white/10 hover:border-gold/40 rounded-xl p-6 cursor-pointer transition-all group">
              <MdUploadFile className={`text-2xl transition-colors ${file ? 'text-gold' : 'text-gray-500 group-hover:text-gold'}`} />
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full flex items-center justify-center gap-2 bg-gold hover:bg-yellow-400 disabled:opacity-60 disabled:cursor-not-allowed text-navy font-montserrat font-bold text-sm uppercase tracking-wider py-3.5 rounded-xl transition-all"
        >
