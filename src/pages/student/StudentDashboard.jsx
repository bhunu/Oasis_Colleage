...
import { collection, getDocs, getDoc, query, where, orderBy, limit, onSnapshot, doc } from 'firebase/firestore'
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    /* non-blocking — read subject marks from the real nested path */
    ;(async () => {
      try {
        const rawTerm = portalSettings?.currentTerm ?? getCurrentTerm().number
        const termNum = parseTermNumber(rawTerm)
        const termId  = `term-${termNum}-${new Date().getFullYear()}`
        const classId = (studentData.class || '').toLowerCase().replace(/\s+/g, '-')
        const snap    = await getDoc(doc(db, 'schools', SCHOOL_ID, 'terms', termId, 'classes', classId, 'students', studentData.regNumber))
        if (snap.exists()) {
          const subjects = snap.data().subjects || {}
          const rows = Object.entries(subjects)
            .filter(([, mark]) => mark !== null && mark !== undefined && mark !== '')
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      {/* Welcome banner */}
      <div className="bg-gradient-to-r from-navy-800 to-navy-light border border-white/10 rounded-xl p-6">
        <p className="text-[10px] font-semibold text-gold/70 uppercase tracking-widest font-montserrat mb-1">
          {termLabel} · {termYear}
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {/* Fee status card */}
        <div className={`${CARD} cursor-pointer hover:border-gold/30 transition`} onClick={() => navigate('/student/fees')}>
          ...
          {termFees > 0 && (
            <div className="mt-3">
              <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-gold rounded-full transition-all" style={{ width: `${Math.min(paidPct, 100)}%` }} />
              </div>
            ...
            <>
              ...
              <div className="mt-2">
                ...
                <div className="relative h-1.5 bg-white/10 rounded-full overflow-visible mt-3 mb-1">
                  ...
                  {/* Threshold marker */}
                  <div
                    className="absolute top-1/2 -translate-y-1/2 w-0.5 h-3 bg-gold rounded-full"
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <button
            ...
            className="bg-navy-800 border border-white/10 hover:border-gold/30 hover:bg-gold/5 rounded-xl py-4 flex flex-col items-center gap-2 transition group"
          >
            <Icon className="text-gray-500 group-hover:text-gold text-2xl transition" />
      ...
      {receipts.length > 0 && (
        <div className={CARD}>
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => navigate('/student/fees')} className="text-xs text-gold font-montserrat hover:text-yellow-300 transition">View all</button>
          </div>
      ...
      {notifications.length > 0 && (
        <div className={CARD}>
          ...
          <div className="space-y-2">
            {notifications.map(n => (
              <div key={n.id} className={`p-3 rounded-lg border ${n.read ? 'bg-transparent border-white/5' : 'bg-gold/5 border-gold/20'}`}>
