  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl px-5 py-3 text-amber-300 font-montserrat text-xs">
        View only — Bursar can view fee accounts and payment history. To update a payment, use Receive Payment.
      </div>
        ...
        <>
          ...
          <div className={CARD}>
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-playfair font-semibold text-white text-lg">{selected.name}</h3>
                <p className="text-sm text-gray-400 font-montserrat">{selected.regNumber} · {selected.class || selected.form || '—'}</p>
              </div>
              {account ? (
          ...
          {account && (
            <div className={CARD}>
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  onClick={() => navigate(`/bursar/receive-payment?student=${selected.id}`)}
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    {ledger().length === 0 && (
                      <tr><td colSpan={5} className="py-6 text-center text-sm text-gray-500 font-montserrat">No transactions recorded</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {!selected && (
        <div className={`${CARD} text-center py-12`}>
          <p className="text-gray-500 font-montserrat text-sm">Search for a student to view their fee account and payment history.</p>
        </div>
      )}
    </div>
  )
